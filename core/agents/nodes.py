...
import numpy as np
import faiss
from _utils import call_llm, get_embedding, fixed_size_chunk
...
class CreateIndexNode(Node):
    ...
    def exec(self, embeddings):
        ...
        embeddings = embeddings.squeeze(1)  # → (5, 1024)
        # embeddings = embeddings.astype('float32')  # dtype 要求
        dimension = embeddings.shape[1]  # = 1024
        index = faiss.IndexFlatL2(dimension)  # index.d == 1024
        index.add(embeddings)  # 现在 n=5, d=1024，和 index.d=1024 相符
        return index
