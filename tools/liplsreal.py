...
def warm_up(batch_size,model,modelres):
    ...
    hn = torch.zeros(2, 1, 512).to(device, dtype=weight_dtype)
    cn = torch.zeros(2, 1, 512).to(device, dtype=weight_dtype)
    img_batch = torch.ones(1, 6, modelres, modelres).to(device, dtype=weight_dtype)
    mel_batch = torch.ones(1, 10, 1024).to(device, dtype=weight_dtype)
...
def inference(quit_event,batch_size,face_list_cycle,audio_feat_queue,audio_out_queue,res_frame_queue,model):
    ...
    while not quit_event.is_set():
        ...
        if is_all_silence:
            ...
            mel_batch = torch.FloatTensor(mel_batch)  #(np.transpose(mel_batch, (0, 3, 1, 2))) #.to(device)

            with torch.no_grad():
                g_frames = []
                for frame in range(batch_size):
                    if infernum % syncnet_T == 0:
                        # 初始化时间步
                        hn = torch.zeros(2, 1, 512).to(device, dtype=weight_dtype)
                        cn = torch.zeros(2, 1, 512).to(device, dtype=weight_dtype)

                    x_frame = img_batch[frame, :, :, :].to(device, dtype=weight_dtype, non_blocking=True)
                    indiv_frame = mel_batch[frame, :, :].to(device, dtype=weight_dtype, non_blocking=True)
                    #print(indiv_frame.shape, x_frame.shape)
                    g, hn, cn = model(indiv_frame.unsqueeze(0), x_frame.unsqueeze(0), hn, cn)
                    g_frames.append(g.to(dtype=weight_dtype))
                    infernum += 1
                pred = torch.stack(g_frames, dim=0).squeeze(1)
    ...
    print('liplsreal inference processor stop')

class LipLsReal(BaseReal):
    @torch.no_grad()
    def __init__(self, opt, model, avatar):
        ...
        self.asr = LightASR(opt,self,self.audio_processor,[0,5])
        self.asr.warm_up()
        
        self.render_event = mp.Event()
    
    def __del__(self):
        print(f'liplsreal({self.sessionid}) delete')
    ...
    def process_frames(self,quit_event,loop=None,audio_track=None,video_track=None):
        ...
        print('liplsreal process_frames thread stop') 
    ...
    def render(self,quit_event,loop=None,audio_track=None,video_track=None):
        ...
        Thread(target=self.main_playthrd, args=(quit_event,)).start()
